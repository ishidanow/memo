...
def send_email(subject, body):
    ...
    to_email = "hideo813072@gmail.com"
...
def check_onion_sites():
    ...
    for onion_url in onion_urls:
        if is_onion_site_accessible(onion_url):
            ...
            not_accessible_sites.append(onion_url)

    if accessible_sites:
        accessible_sites_str = "\n".join(accessible_sites)
        send_email(
            subject="[Access Successful]:Onion Site Access Alert",
            body=f"The following sites are accessible via Tor:\n{accessible_sites_str}"
        )
    if not_accessible_sites:
        not_accessible_sites_str = "\n".join(not_accessible_sites)
        send_email(
            subject="[Access Error]:Onion Site Access Alert",
            body=f"The following sites are not accessible via Tor:\n{not_accessible_sites_str}"
        )
